...
import { ActionRow, ActionRowBuilder, ApplicationCommandOptionType, ApplicationCommandType, ButtonBuilder, ButtonComponent, ButtonStyle, ChatInputApplicationCommandData, ChatInputCommandInteraction, Collection, Embed, EmbedBuilder, Message, Snowflake, TextChannel } from "discord.js";
    ...
    async interactiveExecute(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        if (response.customId === approveLiteral) {
            ...
            await response.update({
                ...
                embeds: [emb],
                components: [new ActionRowBuilder<ButtonBuilder>().addComponents(approveBtn.setDisabled(true))]
            });
        }
        else
            await response.update({
